...
```smile
Да. Юмор приветствуется в любом виде. Меня тоже можно и нужно подкалывать, с самооценкой у меня всё в порядке. ;)
```
## Репозитории:
...
Надо сделать иначе:
- Написать скрипт get_custom_modules.sh, который будет делать всё тоже самое, но только в самом начале работы скрипта
...
- Соответственно, нужен отдельный workflow для добавления кастомных модулей в config/modules.json. В его inputs:
```json
      "name": "",
      "nickname": "",
      "repo": "",
